...
### 주간 랭킹 — 자체 인기 엔진 (JustWatch 없음)

**외부 랭킹 API 없이** 주간 TOP10을 만듭니다. JustWatch 포기 → 독립적·무료·중단 리스크 없음.

- **후보:** TMDB 한국 OTT 인기(넷플릭스/디즈니+ 등) 상위 20편
- **순위 결정:** 작품명 + "예고편" YouTube 검색 → **조회수 합** + **공식 채널 가중치(1.5배)** 로 점수 계산 → 점수 순 TOP10
- **체감:** "넷플릭스 공식 TOP10"보다 "요즘 많이 보는 콘텐츠"에 가깝고, 한국 사용자에게 직관적
- **추가 가능(2차):** Google Trends 검색량, 내부 사용자 행동(작품 클릭수·영상 재생수·북마크)으로 점수 보정
- **Cron:** `GET /api/cron/sync-weekly-ranking` (매주 일요일 23:59 KST)
