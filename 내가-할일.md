...
| **2** | **(선택) Cron 영상 수집** | Vercel Cron이 매일 `/api/cron/sync-videos` 호출하도록 되어 있음. 환경 변수에 CRON_SECRET 맞게 넣었으면 자동 수집됨. works에 데이터가 있어야 영상이 수집됨. |
...
**요약:** 지금은 **works에 작품만 계속 넣어 주면** 사이트가 채워집니다. Cron 켜두면 works 기준으로 YouTube 영상도 자동으로 붙습니다.

---

### 작품 자동 수집 (TMDB)

TMDB 트렌딩 영화·드라마를 가져와 **works**에 자동으로 넣는 API가 추가되어 있습니다.

- **호출 방법 (수동 1회)**  
  - URL: `GET https://본인도메인.com/api/cron/sync-works`  
  - Header: `Authorization: Bearer (CRON_SECRET 값)`  
  - Postman이나 브라우저에서 호출하면 영화 10편 + TV 10편이 works에 추가/갱신됩니다.
- **필수:** Vercel(또는 .env.local)에 **TMDB_API_KEY**가 들어 있어야 합니다.
- **자동 실행:** Vercel Cron에 포함되어 있어, **매주 일요일 새벽 4시(UTC)** 에 한 번 실행됩니다. (원하면 `vercel.json`에서 스케줄 변경 가능)
